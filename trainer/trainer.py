...
class Trainer:
	def __init__(self, config: Config, model, criterion, optimizer, device, train_dataloader,
	             eval_dataloader=None, writer: Writer = None):
		self.config = config
	...
	def train(self):
		...
		for it in pbar:
			...
			pbar.set_description(f"Train: Iter - {it} Loss - {float(self.train_loss.val):.8f}")
